...
from functions.svd_ddnm import ddnm_diffusion,  ddnm_EquS_diffusion,ddnm_mDSAR_diffusion, ddnm_DSAR_diffusion
...
class Diffusion(object):
    ...
    def simplified_ddnm_plus(self, model, cls_fn):
        ...
        for x_orig, classes in pbar:
            ...
            # Metrics below expect finite tensors; NaNs can appear if the sampler becomes unstable.
            # Keep images in [0, 1] for saving/PSNR/SSIM, but clean NaNs/Infs to avoid crashing.
            x = [inverse_data_transform(config, xi) for xi in x]
            x = [
                torch.nan_to_num(xi, nan=0.0, posinf=1.0, neginf=0.0).clamp(0.0, 1.0)
                for xi in x
            ]
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        for x_orig, classes in pbar:
            ...
            with torch.no_grad():
            # ------ 1. 你自己的方法阵营 ------
                if getattr(self.args, 'EquS', False):
                    x, _ = ddnm_EquS_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, cls_fn=cls_fn, classes=classes, config=config)
                
                elif getattr(self.args, 'DSAR', False):
                    x, _ = ddnm_DSAR_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, cls_fn=cls_fn, classes=classes, config=config)
                elif getattr(self.args, 'mDSAR', False):
                    x, _ = ddnm_mDSAR_diffusion(x, model, self.betas, self.args.eta, A_funcs, y, cls_fn=cls_fn, classes=classes, config=config)    
...
# ==========================================
            # 🚀 第二步：恢复图片原始命名 (从 txt 读取)
            # ==========================================
            with open("exp/SAR_val_100.txt", "r") as f:
                file_lines = [line.strip().split()[0] for line in f.readlines()]

            for j in range(x[0].size(0)):
                # 获取它在 txt 里的绝对顺序
                global_idx = idx_so_far + j
                orig_name = os.path.basename(file_lines[global_idx])
                new_name = orig_name.replace(".jpg", ".png").replace(".JPEG", ".png")
                
                # 用真实的原来名字保存
                tvu.save_image(
                    x[0][j], os.path.join(self.args.image_folder, new_name)
                )
                pred = x[0][j].to(self.device)
                orig = inverse_data_transform(config, x_orig[j]).to(self.device)
                pred = torch.nan_to_num(pred, nan=0.0, posinf=1.0, neginf=0.0).clamp(0.0, 1.0)
                orig = torch.nan_to_num(orig, nan=0.0, posinf=1.0, neginf=0.0).clamp(0.0, 1.0)

                mse = torch.mean((pred - orig) ** 2)
                mse = torch.nan_to_num(mse, nan=1.0, posinf=1.0, neginf=1.0)
                psnr = 10 * torch.log10(1.0 / torch.clamp(mse, min=1e-10))
                avg_psnr += psnr

                ssim_i = ssim(
                    pred.detach().cpu().numpy(),
                    orig.detach().cpu().numpy(),
                    data_range=1.0,
                    channel_axis=0,
                )

                # TorchMetrics LPIPS expects inputs in [-1, 1]
                pred_lpips = (pred * 2.0 - 1.0).clamp(-1.0, 1.0)
                orig_lpips = (orig * 2.0 - 1.0).clamp(-1.0, 1.0)
                lpips_value = loss_fn(
                    pred_lpips.unsqueeze(0),
                    orig_lpips.unsqueeze(0),
                ).item()
