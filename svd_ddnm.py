...
# ==========================================
                # 🚀 YOUR NOVELTY: SAR-Specific TV Regularization (L2 版本)
                # ==========================================
                with torch.enable_grad():
                    x0_t_opt = x0_t.detach().clone().requires_grad_(True)
                    
                    # 💡 核心大招：换成平方差！让梯度丝滑收敛，彻底告别方块噪声！
                    diff_i = torch.sum((x0_t_opt[:, :, :, 1:] - x0_t_opt[:, :, :, :-1]) ** 2)
                    diff_j = torch.sum((x0_t_opt[:, :, 1:, :] - x0_t_opt[:, :, :-1, :]) ** 2)
                    tv_loss = diff_i + diff_j
                    
                    grad_tv = torch.autograd.grad(outputs=tv_loss, inputs=x0_t_opt)[0]
                
                # 因为 L2 梯度极其丝滑，我们把力度设置在 0.01（你可以视情况调到 0.05 或 0.005）
                lambda_tv = 0.01
